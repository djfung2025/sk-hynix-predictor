...
1. **获取 GitHub 个人访问令牌 (Personal Access Token - PAT)**：
   - 访问 GitHub，点击页面右上角的**个人头像**，在下拉菜单中选择 **Settings**（这是个人账户设置，请注意**不要**在具体的项目仓库页面中找 Settings）。
   - 在个人设置页面左侧侧边栏的最下方，点击 **Developer settings**。
   - 依次点击 **Personal access tokens -> Fine-grained tokens**。
   - 点击右上角的 **Generate new token**。
   - 将令牌命名为 `hynix-sync-token`，在 **Repository access** 中选择 `Only select repositories`，然后下拉选择您刚刚为该项目创建的仓库（如 `sk-hynix-predictor`）。
   - 在下面的 **Permissions -> Repository permissions** 中，找到 **Contents**，将其权限设置为 **Read and write**。
   - 滚动到页面底部点击 **Generate token**，并复制生成的令牌（形如 `github_pat_xxxx`）。
